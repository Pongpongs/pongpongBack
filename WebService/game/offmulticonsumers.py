...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        # 계산식
        if message == 'q':
            GameConsumer.play_bar1_position['x'] = max(
        ...
        if message == 'i':
            GameConsumer.play_bar2_position['x'] = max(
        ...
        if message == 'z':
            GameConsumer.play_bar4_position['y'] = max(
                -9, GameConsumer.play_bar4_position['y'] - 0.4)
        elif message == 'c':
            GameConsumer.play_bar4_position['y'] = min(
                9, GameConsumer.play_bar4_position['y'] + 0.4)
            
        if message == 'b':
            GameConsumer.play_bar3_position['y'] = max(
                -9, GameConsumer.play_bar3_position['y'] - 0.4)
        elif message == 'm':
            GameConsumer.play_bar3_position['y'] = min(
                9, GameConsumer.play_bar3_position['y'] + 0.4)
    ...
    async def _update_ball_position(self):
        ...
        if GameConsumer.play_bar3_position['x'] - ball_radius < GameConsumer.ball_position['x'] < GameConsumer.play_bar3_position['x'] + ball_radius \
                and GameConsumer.play_bar3_position['y'] - bar_width / 2 < GameConsumer.ball_position['y'] < GameConsumer.play_bar3_position['y'] + bar_width / 2:
            GameConsumer.ball_velocity['x'] *= -1  # y 방향 반전

        if GameConsumer.play_bar4_position['x'] - ball_radius < GameConsumer.ball_position['x'] < GameConsumer.play_bar4_position['x'] + ball_radius \
                and GameConsumer.play_bar4_position['y'] - bar_width / 2 < GameConsumer.ball_position['y'] < GameConsumer.play_bar4_position['y'] + bar_width / 2:
            GameConsumer.ball_velocity['x'] *= -1  # y 방향 반전
        ...
        if GameConsumer.ball_position['y'] <= -10 or GameConsumer.ball_position['y'] >= 10:
            ...
            GameConsumer.play_bar3_position = {'x': 9, 'y': 0}
            GameConsumer.play_bar4_position = {'x': -9, 'y': 0}
            await asyncio.sleep(2)

        if GameConsumer.ball_position['x'] <= -10 or GameConsumer.ball_position['x'] >= 10:
            # collision on upper side wall, player2 score up
            if GameConsumer.ball_position['x'] > 10:
                GameConsumer.score_player3 += 1
            # collision on bottom side wall,player1 score up
            elif GameConsumer.ball_position['x'] < -10:
                GameConsumer.score_player4 += 1
        ...
        if GameConsumer.score_player3 == GameConsumer.game_over_score:
            GameConsumer.game_over_flag = True
            GameConsumer.game_winner_local = 3

        if GameConsumer.score_player4 == GameConsumer.game_over_score:
            GameConsumer.game_over_flag = True
            GameConsumer.game_winner_local = 4
        ...
        await self.channel_layer.group_send(
            self.room_group_name,
            {
                ...
                'play_bar3_position': GameConsumer.play_bar3_position,
                'play_bar4_position': GameConsumer.play_bar4_position,
                'ball_position': GameConsumer.ball_position,
                'score_player1': GameConsumer.score_player1,
                'score_player2': GameConsumer.score_player2,
                'score_player3': GameConsumer.score_player3,
                'score_player4': GameConsumer.score_player4,
    ...
    async def ball_position_updater(self):
        ...
        GameConsumer.score_player3 = 0
        GameConsumer.score_player4 = 0
