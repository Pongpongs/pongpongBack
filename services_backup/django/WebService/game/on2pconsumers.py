...
class GameConsumer(AsyncWebsocketConsumer):
	...
	async def connect(self):
		...
		if self.game_state['fullbang_flag'] == True:
			print("FLAG IS TRUE!!!")
	...
	#             })
	#         # await asyncio.sleep(3)  # 클라이언트 연결 종료 전 3초 대기
	...
	# 		self.cleanup_game()
	async def disconnect(self, close_code):
		if self.game_state['fullbang_flag'] and self.game_state['connected_clients_count'] > 1:
			self.game_state['connected_clients_count'] -= 1
			if self.game_state['connected_clients_count'] < 2:
				await self.channel_layer.group_send(
					self.room_group_name,
	...
	async def cleanup_game(self):
		...
		self.game_state['fullbang_flag'] = False
